...
ALLOWED_RAPID_MLX_ENV_VARS: frozenset[str] = frozenset(
    {
        ...
        # Per-temperature half of the same opt-in: whether a family that has
        # qualified copying under sampling uses it for temperature > 0
        # requests. It selects a draft source inside an already-selected
        # model's speculative decode, and the emitted distribution is the
        # target's either way, so it cannot change which model, parser, tier
        # or lane serves the request.
